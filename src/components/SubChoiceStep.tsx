    ...
    <motion.div
          ...
          {options.map((opt, i) => {
            ...
            return (
              <motion.button
                ...
                className={`p-4 rounded-xl text-left text-xl transition-all duration-200 border-2 ${isSelected ? 'border-rose bg-rose/10 text-rose font-bold shadow-sm' : 'border-transparent glass-card text-brown hover:border-rose/30'}`}>
      ...
      <div className="pt-4 mt-auto">
        <button
          ...
          disabled={!selected}
          className={`w-full py-4 rounded-full font-serif text-xl font-medium transition-all duration-300 ${selected ? 'bg-rose text-white shadow-soft hover:scale-[1.02] active:scale-[0.98]' : 'bg-brown/10 text-brown/40 cursor-not-allowed'}`}>
